...
export default function EditProductPage({ params }: { params: Promise<{ id: string }> }) {
    ...
    const fetchTemplate = async (id: string) => {
        try {
            ...
            if (res.ok) {
                ...
                // materials in data might be objects or strings
                const matIds = data.materials.map((m: any) => typeof m === 'object' ? m._id : m);
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        setLoading(true);
        try {
            const res = await fetch(`/api/products/${templateId}`, {
    ...
    return (
        <div className="space-y-8 max-w-5xl mx-auto">
            <div className="flex items-center space-x-4">
                ...
                <div>
                    <h2 className="text-3xl font-bold text-white">Редактирование шаблона</h2> // Changed Title
                    <p className="text-gray-400">Измените параметры и формулу</p>
                </div>
            </div>
